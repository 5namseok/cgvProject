  ...
  <div class="container">
		...
		<table width="700" border="1" bordercolor="gray">
			<tr height = "40">
				<td colspan="5" align="right">
					<input type="button" value="글쓰기" onclick="location.href ='${contextPath}/QNAWriteController.do'">
				</td>
			</tr>
	...
	<c:if test="${count>0}">
		<c:if test="${blockStart > page}">
			<a href="${contextPath}/QNAListController.do?pageNum=${blockStart-page}">[이전]</a>
		</c:if>
		...
		<c:if test="${blockEnd < pageCount}">
			<a href="${contextPath}/QNAListController.do?pageNum=${blockStart+page}">[다음]</a>
		</c:if>
	</c:if>
    </div>
  </div>
