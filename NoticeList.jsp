  ...
  <div class="container">
      ...
      <table width="700" border="1" bordercolor="skyblue">
		<tr height="40" align="right">
          ...
          String ID_getdata=(String)ID_get;
            ...
            location.href='NoticeCreateCon.do'
          })
				</script>
			</td>
		</tr>
		...
		<tr height="40">
			...
			<td width="320" align="left"><a href='NoticeInfoCon.do?num=${bean.num}'>[${bean.num}] ${bean.subject}</a></td>
	...
	<c:if test="${count>0}">
		<c:if test="${blockStart > page}">
			<a href="NoticeListCon.do?pageNum=${blockStart-page}">[이전]</a>
		</c:if>
		<c:forEach var="i" begin="${blockStart}" end="${blockEnd}">
				<a href="NoticeListCon.do?pageNum=${i}" style ="text-decoration:none">[${i}]</a>
		</c:forEach>
		<c:if test="${blockEnd < pageCount}">
			<a href="NoticeListCon.do?pageNum=${blockStart+page}">[이후]</a>
		</c:if>
	</c:if>
    </div>
  </div>
